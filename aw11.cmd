@echo off
title Aktivasi Windows 11 Tanpa Software !
cls
setlocal ENABLEDELAYEDEXPANSION

echo.
echo =====================================================================================
echo    Windows 11 Activation Script 
echo =====================================================================================
echo.
echo    - Windows 11 Home
echo    - Windows 11 Pro
echo    - Windows 11 Education
echo    - Windows 11 Enterprise
echo.
echo.
echo.
echo =====================================================================================
echo    [PROSES] Mengaktivasi Windows 11 Anda, silahkan tunggu...
echo =====================================================================================
echo.

:: Periksa hak akses administrator
net session >nul 2>&1
if !errorLevel! GTR 0 (
    echo.
    echo [GALAT] Aktivasi tidak bisa dilanjutkan.
    echo Skrip ini harus dijalankan sebagai administrator.
    echo Silakan klik kanan pada file .cmd dan pilih "Run as administrator".
    echo.
    goto halt
)

:: Periksa apakah Windows Script Host diaktifkan
cscript //nologo slmgr.vbs /ckms >nul || goto wshdisabled

:: Hapus kunci produk yang ada
cscript //nologo slmgr.vbs /upk >nul
cscript //nologo slmgr.vbs /cpky >nul

set i=1
wmic os | findstr /I "enterprise" >nul
if !errorlevel! EQU 0 (
    echo [PROSES] Mendeteksi Windows 11 Enterprise...
    cscript //nologo slmgr.vbs /ipk NPPR9-FWDCX-D2C8J-H872K-2YT43 >nul || cscript //nologo slmgr.vbs /ipk DPH2V-TTNVB-4X9Q3-TJR4H-KHJW4 >nul || cscript //nologo slmgr.vbs /ipk YYVX9-NTFWV-6MDM3-9PT4T-4M68B >nul || cscript //nologo slmgr.vbs /ipk 44RPN-FTY23-9VTTB-MP9BX-T84FV >nul || cscript //nologo slmgr.vbs /ipk WNMTR-4C88C-JK8YV-HQ7T2-76DF9 >nul || cscript //nologo slmgr.vbs /ipk 2F77B-TNFGY-69QQF-B8YKP-D69TJ >nul || cscript //nologo slmgr.vbs /ipk DCPHK-NFMTC-H88MJ-PFHPY-QJ4BJ >nul || cscript //nologo slmgr.vbs /ipk QFFDN-GRT3P-VKWWX-X7T3R-8B639 >nul || cscript //nologo slmgr.vbs /ipk M7XTQ-FN8P6-TTKYV-9D4CC-J462D >nul || cscript //nologo slmgr.vbs /ipk 92NFX-8DJQP-P6BBQ-THF9C-7CG2H >nul
    goto skms
)

wmic os | findstr /I "home" >nul
if !errorlevel! EQU 0 (
    echo [PROSES] Mendeteksi Windows 11 Home...
    cscript //nologo slmgr.vbs /ipk TX9XD-98N7V-6WMQ6-BX7FG-H8Q99 >nul || cscript //nologo slmgr.vbs /ipk 3KHY7-WNT83-DGQKR-F7HPR-844BM >nul || cscript //nologo slmgr.vbs /ipk 7HNRX-D7KGG-3K4RQ-4WPJ4-YTDFH >nul || cscript //nologo slmgr.vbs /ipk PVMJN-6DFY6-9CCP6-7BKTT-D3WVR >nul
    goto skms
)

wmic os | findstr /I "education" >nul
if !errorlevel! EQU 0 (
    echo [PROSES] Mendeteksi Windows 11 Education...
    cscript //nologo slmgr.vbs /ipk NW6C2-QMPVW-D7KKK-3GKT6-VCFB2 >nul || cscript //nologo slmgr.vbs /ipk 2WH4N-8QGBV-H22JP-CT43Q-MDWWJ >nul
    goto skms
)

wmic os | findstr /I "11 pro" >nul
if !errorlevel! EQU 0 (
    echo [PROSES] Mendeteksi Windows 11 Pro...
    cscript //nologo slmgr.vbs /ipk W269N-WFGWX-YVC9B-4J6C9-T83GX >nul || cscript //nologo slmgr.vbs /ipk MH37W-N47XK-V7XM9-C7227-GCQG9 >nul || cscript //nologo slmgr.vbs /ipk NRG8B-VKK3Q-CXVCJ-9G2XF-6Q84J >nul || cscript //nologo slmgr.vbs /ipk 9FNHH-K3HBT-3W4TD-6383H-6XYWF >nul || cscript //nologo slmgr.vbs /ipk 6TP4R-GNPTD-KYYHQ-7B7DP-J447Y >nul || cscript //nologo slmgr.vbs /ipk YVWGF-BXNMC-HTQYQ-CPQ99-66QFC >nul
    goto skms
)

goto notsupported

:skms
if !i! GTR 5 (goto busy) else if !i! LEQ 5 (set KMS=s1.mshost.pro)
echo [PROSES] Menghubungkan ke server KMS...
cscript //nologo slmgr.vbs /skms !KMS!:1688 >nul

:ato
echo.
echo =====================================================================================
echo.
cscript //nologo slmgr.vbs /ato | find /i "successfully" && (
    echo.
    echo =====================================================================================
    echo.
    echo [BERHASIL] Produk Anda berhasil diaktivasi!
    echo.
    echo # Please support this project: https://trakteer.id/ariphx
    echo.
    timeout /t 7 >nul
    explorer "https://trakteer.id/ariphx"
    echo.
    goto halt
) || (
    echo Sepertinya butuh waktu lebih, silahkan tunggu...
    echo.
    echo.
    set /a i+=1
    timeout /t 10 >nul
    goto skms
)

:notsupported
echo =====================================================================================
echo.
echo [GALAT] Maaf, Windows 11 Anda tidak didukung oleh skrip ini.
echo.
goto halt

:wshdisabled
echo =====================================================================================
echo.
echo [GALAT] Maaf, aktivasi gagal karena Windows Script Host dinonaktifkan.
echo.
goto halt

:busy
echo =====================================================================================
echo.
echo [GALAT] Maaf, aktivasi gagal karena koneksi internet Anda tidak stabil.
echo.
goto halt

:halt
cd %~dp0
del %0 >nul
pause >nul
